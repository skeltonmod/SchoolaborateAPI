...
use Deyji\Manage\Mail\VerificationTokenGeneratedAgent;
use Exception;
...
class UserVerification{
	...
	public function send(AuthenticatableContract $user, $subject = null, $from = null, $name = null, $role = null){
		$this->emailVerificationLink($user, $subject, $from, $name, $role);
	}

	protected function emailVerificationLink(AuthenticatableContract $user, $subject = null, $from = null, $name = null, $role = null){
		if($role === "Agent"){
			return Mail::to($user)->send(new VerificationTokenGeneratedAgent($user, $subject, $from, $name, $role));
		}else{
			return Mail::to($user)->send(new VerificationTokenGenerated($user, $subject, $from, $name, $role));
		}
	}
